"""
JARVIS AI Operating System - Browser Automation Executor.
=========================================================
Playwright Chromium browser automation service with:
- Web navigation, multi-tab coordination, input filling, and element clicking
- Content extraction and page summarization
- Google search with resilient lightweight DuckDuckGo HTML fallback
- Autonomous Web Agent perceive-decide-act-observe reasoning loop
- Deep research and YouTube music playback execution
"""
...
class BrowserExecutor:
    ...
    async def start(self) -> None:
        """Initialize the Playwright Chromium browser instance."""
        if self._started:
        ...
        try:
            ...
            self._started = True
            logger.info("BrowserExecutor started (Chromium)")
    ...
    async def stop(self) -> None:
        ...
        try:
            ...
            self._browser = None
            self._playwright = None
            logger.info("BrowserExecutor stopped")
        except Exception as e:
    ...
    async def search_web(self, query: str) -> str:
        ...
        # Fallback using DuckDuckGo HTML search
        try:
            ...
            results = []
            blocks = re.findall(r'<div class="result__body">(.*?)(?:</div>\s*</div>|</div>\s*<div|\Z)', html, re.DOTALL)
            if not blocks:
                blocks = [html]
    ...
    async def take_page_screenshot(self) -> Optional[Image.Image]:
        """Take a screenshot of current browser viewport."""
        await self._ensure_started()
        try:
    ...
    async def play_music(self, query: str) -> str:
        ...
        try:
            ...
            if video_url:
                logger.info(f"Opening YouTube video: {video_url}")
                webbrowser.open(video_url)
                return f"Playing '{query}' on YouTube: {video_url}"
        except Exception as e:
            logger.warning(f"Failed to extract direct YouTube video link headlessly: {e}. Opening search directly.")
...
BrowserService = BrowserExecutor
